...
describe('mapshaper-commands.js', function() {
  describe('Issue #264 applyCommands()', function() {
    it('should throw error if input is a file path, not file content', function(done) {
      mapshaper.applyCommands(
        '-i input.shp -o out.json',
        { 'input.shp': 'test/data/two_states.shp' },
        function(err, output) {
          assert(!!err)
          done()
        }
  ...
  describe('context tests', function() {
    it('context vars are reset after commands run', function(done) {
      ...
      api.runCommands(cmd, function(err) {
        ...
        assert.deepEqual(api.internal.getStateVar('input_files'), [
          'test/data/three_points.geojson'
        ])
      })
    })
  })
  ...
  describe('layer naming tests', function() {
    it('Fix: name= option of second dataset ignored', function(done) {
      ...
      api.applyCommands(
        '-i a.json name=one -i b.json name=two -o target=*',
        input,
        function(err, output) {
    ...
    it('"-innerlines + name=innerlines" creates a named output layer', function(done) {
      var cmds = api.internal.parseCommands(
        states_shp + ' -innerlines + name=innerlines'
      )
      api.internal.runParsedCommands(cmds, null, function(err, catalog) {
  ...
  describe('applyCommands()', function() {
    ...
    it('Promise returns data', function() {
      var input = [{ foo: 'bar' }]
      return mapshaper
        .applyCommands('-i foo.json -o', { 'foo.json': input })
        .then(function(data) {
    ...
    it('works with -clip command', function(done) {
      ...
      api.applyCommands(
        '-i poly.json -clip clip_poly.json -o',
        { 'poly.json': poly, 'clip_poly.json': clip_poly },
        function(err, output) {
          assert(!!output && !err)
          done()
        }
    ...
    it('output from sequentially processed files is combined', function(done) {
      ...
      api.applyCommands(
        '-i data.csv data2.csv -rename-fields FID=id -o',
        input,
        function(err, output) {
    ...
    it('when two files are processed in sequence and second file triggers error, no output is generated', function(done) {
      ...
      api.applyCommands('-i data.csv data2.json -o', input, function(
        err,
        output
      ) {
    ...
    it('merge multiple files', function(done) {
      ...
      api.applyCommands(
        '-i data.csv data2.csv combine-files -merge-layers -o merged.csv',
        input,
        function(err, output) {
    ...
    it('rename, convert csv', function(done) {
      ...
      api.applyCommands('-i data.csv -o data2.tsv', input, function(
        err,
        output
      ) {
    ...
    it('converts csv to geojson points', function(done) {
      ...
      api.applyCommands(
        '-i points.csv -points x=lng y=lat -o format=geojson',
        { 'points.csv': input },
        function(err, output) {
      ...
      var geojson = {
        ...
        features: [
          ...
          },
          {
            ...
            properties: { name: 'Iguazu Falls' }
          }
        ]
      }
      api.applyCommands(
        '-i points.shp -filter-fields name -o -o format=geojson',
        input,
        function(err, output) {
    ...
    it('convert GeoJSON points to TopoJSON', function(done) {
      ...
      api.applyCommands(
        '-i data.json name=point -o format=topojson precision=1',
        { 'data.json': geojson },
        function(err, output) {
  ...
  describe('runCommands()', function() {
    ...
    it('Error: -join missing a file', function(done) {
      mapshaper.runCommands('-i ' + states_shp + ' -join oops.json', function(
        err
      ) {
        assert.equal(err.name, 'UserError')
        done()
  ...
  describe('testCommands()', function() {
    it('multiple input files are processed in sequence', function(done) {
      mapshaper.internal.testCommands(
        '-i test/data/three_points.geojson test/data/one_point.geojson',
        function(err, dataset) {
          assert.deepEqual(dataset.info.input_files, [
            'test/data/one_point.geojson'
          ])
    ...
    it('Callback returns dataset for imported file', function(done) {
      mapshaper.internal.testCommands('-i ' + states_shp, function(
        err,
        dataset
      ) {
